...
class DenseResidualBlock(nn.Module):
    """Lightweight dense residual block used inside RRDB.

    Uses 3 small conv layers with dense connections and a final 1x1 that reduces
    channels back to in_channels. Returns x + 0.2 * F(x) (residual scaling).
    """
    def __init__(self, in_channels, growth=32):
        super().__init__()
        g = int(growth)
        self.conv1 = nn.Conv2d(in_channels, g, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(in_channels + g, g, kernel_size=3, padding=1)
        self.conv3 = nn.Conv2d(in_channels + 2 * g, g, kernel_size=3, padding=1)
        self.conv4 = nn.Conv2d(in_channels + 3 * g, in_channels, kernel_size=3, padding=1)
        self.act = nn.LeakyReLU(0.2, inplace=True)

    def forward(self, x):
        c1 = self.act(self.conv1(x))
        c2 = self.act(self.conv2(torch.cat([x, c1], dim=1)))
        c3 = self.act(self.conv3(torch.cat([x, c1, c2], dim=1)))
        c4 = self.conv4(torch.cat([x, c1, c2, c3], dim=1))
        return x + 0.2 * c4


class RRDB(nn.Module):
    """Residual-in-Residual Dense Block (stack of DenseResidualBlock).

    Typical configuration uses nb=3 (as in ESRGAN). We wrap several DenseResidualBlocks
    and apply a residual scaling at the RRDB level as well.
    """
    def __init__(self, in_channels, growth=32, nb=3):
        super().__init__()
        self.blocks = nn.Sequential(*[DenseResidualBlock(in_channels, growth) for _ in range(nb)])
...
class Generator(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, ngf, ch_mult=(1, 2, 4, 8), num_res_blocks=3):
        ...
        # Middle block replaced with a stack of 2 RRDB blocks (stronger residual/dense representation)
        growth = min(32, (ngf * ch_mult[-1]) // 2)
        self.mid_block = nn.Sequential(
            RRDB(ngf * ch_mult[-1], growth=growth, nb=3),
            RRDB(ngf * ch_mult[-1], growth=growth, nb=3)
        )
