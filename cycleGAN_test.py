...
def test(
    ...
    for i, index in enumerate(sampled_indices):
        quarter = np.load(quarter_files[index])
        full = np.load(full_files[index])
        output = np.load(output_files[index])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='test_results')
